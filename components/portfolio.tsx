...
import {
  ...
  ChevronDown, CircleHelp, Clock3, Coins, Landmark, ListFilter, Menu,
  MoreHorizontal, Plus, RefreshCw, Search, ShieldCheck, Star, UserRound,
  WalletCards, X,
} from "lucide-react";
  ...
  return (
    <main className="app-shell">
      ...
      <header className="main-header">
        <div className="header-inner"><a className="brand" href="#top"><span className="brand-symbol"><i /><i /></span><b>Min Sparing</b></a><nav><a className="active" href="#top">Mine sider</a><a href="#beholdning">Beholdning</a><a href="#datakilder">Datakilder</a><a href="#fordeling">Fordeling</a></nav><div className="header-actions"><button className="transfer" onClick={() => setAdding(true)}>Legg til</button><button aria-label="Varsler"><Bell size={16} /></button><button className="search-button" onClick={() => setAdding(true)}><Search size={15} /> Søk</button><button className="mobile-menu" aria-label="Meny"><Menu size={18} /></button></div></div>
      </header>
      ...
      <div className="nordnet-shell" id="top">
        ...
        <section className="portfolio-grid">
          <div className="left-column">
            <AccountRail active={activeAccount} onChange={setActiveAccount} allTotals={allTotals} totals={accountTotals} onAdd={() => setAdding(true)} />
          ...
          <div className="center-column">
            ...
            <section className="holdings-card" id="beholdning">
              <div className="card-title-row"><div><h2>Beholdning</h2><span>{activeAccount === "all" ? "Alle kontoer" : accountConfig[activeAccount].label} · {visibleHoldings.length} investeringer</span></div><div className="holdings-controls"><button className="add-small" onClick={() => setAdding(true)}><Plus size={14} /> Legg til</button><label><ListFilter size={14} /><select value={sortMode} onChange={(event) => setSortMode(event.target.value as SortMode)}><option value="value">Størst verdi</option><option value="today">Dagens utvikling</option><option value="name">Navn A–Å</option></select><ChevronDown size={13} /></label></div></div>
              <div className="holding-head"><span>Investering</span><span>Konto</span><span>Verdi</span><span>I dag</span><span /></div>
              <div className="holding-list">{sortedHoldings.map((item) => <HoldingRow key={item.id} item={item} onRemove={() => remove(item.id)} onAccountChange={(group) => moveAccount(item.id, group)} />)}{!sortedHoldings.length ? <div className="empty"><p>Ingen investeringer på denne kontoen.</p><button onClick={() => setAdding(true)}>Legg til investering</button></div> : null}</div>
            </section>
          </div>
...
function HoldingRow({ item, onRemove, onAccountChange }: { item: Holding; onRemove: () => void; onAccountChange: (group: AccountGroup) => void }) {
  ...
  return <article className="holding-row"><div className="asset-main"><div className={`asset-icon ${item.kind}`}><Icon size={17} /></div><div><h3>{item.name}</h3><p>{item.platform} · {item.symbol} · <span className={item.delayed ? "delayed-text" : "fresh-text"}>{item.delayed ? "Forsinket NAV" : item.source}</span></p></div></div><div className="account-cell" style={{ "--account-color": accountConfig[account].color } as CSSProperties}><i /><select value={account} onChange={(event) => onAccountChange(event.target.value as AccountGroup)}>{accountOrder.map((group) => <option value={group} key={group}>{accountConfig[group].label}</option>)}</select><ChevronDown size={11} /></div><div className="asset-value"><b>{money.format(value)}</b><small>{number.format(item.units)} andeler</small></div><div className={`asset-change ${hasDaily ? (today >= 0 ? "positive" : "negative") : "waiting"}`}>{hasDaily ? (today >= 0 ? <ArrowUpRight size={15} /> : <ArrowDownRight size={15} />) : <CircleHelp size={14} />}<div><b>{hasDaily ? signedPercent(item.dailyPercent ?? 0) : "Venter på NAV"}</b><small>{hasDaily ? signedMoney(today) : item.priceAsOf ? `NAV ${item.priceAsOf}` : formatTime(item.updatedAt)}</small></div></div><button className="remove" onClick={onRemove} aria-label={`Fjern ${item.name}`}><X size={15} /></button></article>;
}
