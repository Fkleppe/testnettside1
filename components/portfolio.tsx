        ...
        <section className="portfolio-grid">
          ...
          <div className="center-column">
            ...
            <section className="holdings-card" id="beholdning">
              <div className="card-title-row"><div><h2>Beholdning</h2><span>{activeAccount === "all" ? "Alle kontoer" : accountConfig[activeAccount].label} · {visibleHoldings.length} investeringer</span></div><div className="holdings-controls"><button className="add-small" onClick={() => setAdding(true)}><Plus size={14} /> Legg til</button><label><ListFilter size={14} /><select aria-label="Sorter investeringer" value={sortMode} onChange={(event) => setSortMode(event.target.value as SortMode)}><option value="value">Størst verdi</option><option value="today">Dagens utvikling</option><option value="name">Navn A–Å</option></select><ChevronDown size={13} /></label></div></div>
